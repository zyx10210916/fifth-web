      ...
      <a-dropdown v-if="item.children && item.children.length > 0">
        ...
        >
          <div class="info-box">
            ...
            {{ item.title }}
            <a-icon type="down" style="margin-left: 5px; font-size: 1.2rem;" />
          </div>
        </div>
        <template #overlay>
          <a-menu
 ...
 */
/**
 * @desc: dropdown click处理
 * @param e
 * @param parentItem
 * @return {*}
 */
const handleDropdownClick = ({ key }, parentItem: any) => {
  emit('click:dropdown', { key }, parentItem);
};
