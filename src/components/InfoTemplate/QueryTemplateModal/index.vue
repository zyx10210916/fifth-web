<template>
  ...
  >
    <div class="template-form">
      <!-- 显示模板名称 -->
      <div class="form-item">
        <label class="form-label">查询模板名称<span class="required">*</span></label>
        <a-input
      ...
      <div class="form-row">
        ...
        <div class="form-item half-width">
          <label class="form-label">是否常用</label>
          <a-select
              v-model:value="formData.isCommon"
              class="form-select"
          >
            <a-select-option value="是">是</a-select-option>
            <a-select-option value="否">否</a-select-option>
...
interface Emits {
  ...
  (e: 'save', data: { formData: TemplateFormData; metrics: any[]; queryConditions: any }): void;
  (e: 'cancel'): void;
}
...
const handleSave = async (): Promise<void> => {
  ...
  try {
    ...
    // 将共享和是否常用字段从"是"/"否"转换为"1"/"2"
    const transformedFormData = {
      ...formData,
      shared: formData.shared === '是' ? '1' : '2',
      isCommon: formData.isCommon === '是' ? '1' : '2'
    };
    ...
    emit('save', {
      ...
      queryConditions: { ...props.queryConditions }
    });
