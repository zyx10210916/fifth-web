        ...
        <tr>
          ...
          <td>
            ...
            >
              ...
              >
                {{report.TABLE_NUM}}-{{ report.TABLE_NAME }}
              </a-select-option>
            </a-select>
          </td>
...
// 存储当前编辑的记录的额外字段
const editRecordExtraFields = ref<any>({});
// 加载报表数据
...
const handleEdit = (record: any) => {
  ...
  // 保存额外字段
  editRecordExtraFields.value = { ...record };
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 如果是编辑，需要包含ID_和其他可能需要的字段
    if (formData.value.ID_) {
      submitData.ID_ = formData.value.ID_;

      // 如果有额外的字段，也一并添加
      if (editRecordExtraFields.value) {
        Object.keys(editRecordExtraFields.value).forEach(key => {
          // 只添加非空的额外字段，避免覆盖已设置的字段
          if (editRecordExtraFields.value[key] !== undefined &&
              editRecordExtraFields.value[key] !== null &&
              !submitData.hasOwnProperty(key)) {
            submitData[key] = editRecordExtraFields.value[key];
          }
        });
      }
    }
